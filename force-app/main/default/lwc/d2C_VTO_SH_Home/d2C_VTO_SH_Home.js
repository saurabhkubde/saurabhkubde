...
// custom imports
import SILHOUETTE_LOGO_IMAGE from '@salesforce/resourceUrl/D2C_VTO_SH_StoreStyling';
import HOME_PAGE_BACKGROUND_IMAGE from '@salesforce/resourceUrl/D2C_VTO_SH_Home_Page_Image';
...
// constants
const SILHOUETTE_D2C_CATALOG_NAME = 'Silhouette D2C Catalog';
...
export default class D2C_VTO_SH_Home extends LightningElement {
    ...
    // Silhouette logo image
    @track _silhouetteLogo = SILHOUETTE_LOGO_IMAGE + '/icons/D2C_VTO_SilhouetteLogo.svg';
    ...
    async connectedCallback() {
        ...
        let _ImageUrlList = _homePageImageList[0].split(':-');
        ...
        // Split the text for the home page into individual components
        this._textList = this._text.split(',');
        this._scanQRcodeText = this._textList[0];
        this._orText = this._textList[1];
        this._continueWithPosScreenText = this._textList[3];

        this.setTimeOutForBackgroundImage();

        // DVM21-7: get categoryId for redirectionUrl
        this._categoryId = await getCategoryId({ catalogName: SILHOUETTE_D2C_CATALOG_NAME, categoryName: OPTICAL_EYEWEAR_CATEGORY });
