...
export function generatePagesForRange(currentPage, totalPage, maxPageButtons) {
    ...
    } else if (currentPage <= Math.ceil(balanceLength / 2)) {
        pages = getRangeArray(balanceLength, 1);
        pages.push(totalPage);
    } else if (currentPage > totalPage - Math.ceil(balanceLength / 2)) {
        pages = getRangeArray(balanceLength, totalPage - balanceLength + 1);
        pages.unshift(1);
    } else {
        let beginIndex = currentPage - Math.floor(balanceLength / 2);
